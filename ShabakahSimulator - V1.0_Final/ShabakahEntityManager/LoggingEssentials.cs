...
namespace ShabakahEntityManager
{
    public class ExtraParam
    {
        ...
        public override string ToString()
        {
            ...
            //if (!Value.GetType().IsPrimitive && Value.GetType() != typeof(string) && Value.GetType() != typeof(DateTime))
            //    return SerializerHelper.JSSerializeObject(Value);
    ...
    public class LoggingEssentials
    {
        ...
        /*
        public string ConstructIOLogMessage()
        {
            logMessage = LayerName + " " + MethodName + tab;//string.Concat(DateTime.Now.ToString("dd/MM/yyyy hh:mm:ss.ffffff"), tab, MAC, tab, LayerName, tab, MethodName, tab);
            //logMessage += IsInput ? "Input: MAC = " + MAC + tab + "SessionID = " + SessionID + tab : "Output: ";
            if (ExtraParams != null)
            {
                foreach (ExtraParam param in ExtraParams)
                {
                    if (param != null)
                    {
                        //string ParamValue = param.GetType() != typeof(int) && param.GetType() != typeof(string) ? SerializerHelper.JSSerializeObject(param.Value) : param.Value.ToString();
                        logMessage += param.Key + " = " + param.ToString() + tab;
                    }
                }
            }
            return logMessage;
